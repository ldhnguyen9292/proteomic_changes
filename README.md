...
Three criteria are applied to every module in every network, against local sweat
rate (LSR) in the impute 10% workflow:

  1. topology     -- scale-free fit R^2 > 0.8 at the selected soft-power;
  2. module-trait -- |r| >= 0.5 and BH-FDR < 0.05;
  3. MM vs GS     -- |r| > 0.6 and p < 0.05 within the module.

Three modules pass: `PT1/PT2` dimgrey (M1), `PR2/PT2` black (M2) and `PT1/PT2`
lightgrey (M3). Two near-misses are worth knowing: `PT1/PT2 darkgrey` clears
criteria 1-2 but its MM-GS is only 0.162, and the full-network `black` module
clears FDR but has r = 0.485.

> **Note.** The GO/KEGG enrichment and hub-prioritization steps, and the scripts
> that produced them (`enrichment.py`, `hub_prioritization.py`, `hub_enrichment.py`,
> `hub_profile_unbiased.py`, `mmgs_threshold.py`), were removed on 2026-08-03
> along with their outputs. The figures still in `results/wgcna/` that were built
> from them -- `mmgs_threshold_*.png` -- remain as images but can no longer be
> regenerated.
